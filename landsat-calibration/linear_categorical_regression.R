...
cota_data <- na.omit(fread('cotas/COTAS_ESTACOES.csv'))

max_ssc_prediction_by_year <- landsat_serie %>%
   group_by(year(ymd(landsat_serie$landsat_dt)), site_no) %>%
   slice(which.max(ssc_prediction))

cota_data <- mutate(cota_data, EstacaoCodigo=as.character(EstacaoCodigo), Data=dmy(Data)) %>% distinct()

# historical_data<- left_join(landsat_serie, cota_data, by=c('site_no'='EstacaoCodigo', 'landsat_dt' ='Data'))

generatePredictedHistoricalSerieByStation(landsat_serie, cota_data, max_ssc_prediction_by_year)