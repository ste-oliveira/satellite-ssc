...
pluviometric_serie <- getPluviometricHistoricalSerie()
discharge_serie <- getDischargeHistoricalSerie()
ssc_discharge_serie <- getSSCDischargeHistoricalSerie()

ssc_discharge_serie_mensal <- ssc_discharge_serie %>% group_by(month, year) %>%
  summarise(Descarga=max(Descarga), Data=first(Data))
